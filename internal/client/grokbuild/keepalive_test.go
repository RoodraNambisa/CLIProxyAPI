...
func TestTransformKeepaliveSSEFrame(t *testing.T) {
	for _, raw := range []string{
		"event: keepalive\ndata: {\"type\":\"keepalive\"}\n\n",
		"id: 9\r\ndata: {\"type\":\"keepalive\"}\r\n\r\n",
		"event: response.created\ndata: {\"type\":\"response.created\"}\n\n",
	} {
		for _, isGrok := range []bool{false, true} {
			out, changed := TransformKeepaliveSSEFrame([]byte(raw), isGrok)
			wantChanged := isGrok && bytes.Contains([]byte(raw), []byte("keepalive"))
			if changed != wantChanged || (changed && !bytes.Equal(out, KeepaliveSSEComment())) || (!changed && string(out) != raw) {
				t.Fatalf("frame=%q grok=%t changed=%t output=%q", raw, isGrok, changed, out)
			}
		}
	}
}
