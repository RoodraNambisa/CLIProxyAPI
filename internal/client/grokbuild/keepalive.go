...
// TransformKeepaliveSSEFrame handles complete frames in the trusted SSE path,
// where event and data fields arrive together instead of one line at a time.
func TransformKeepaliveSSEFrame(frame []byte, isGrokClient bool) ([]byte, bool) {
	if !isGrokClient {
		return frame, false
	}
	var event string
	...
	if event == "keepalive" || IsKeepalivePayload(bytes.Join(data, []byte{'\n'})) {
		return KeepaliveSSEComment(), true
	}
	return frame, false
}
