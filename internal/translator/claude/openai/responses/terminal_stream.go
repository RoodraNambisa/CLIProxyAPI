...
func (st *claudeToResponsesState) noteOutputIndex(index int, nextSeq func() int) [][]byte {
	if _, exists := st.OutputIndexes.ByBlock[index]; exists {
		return nil
	}
	var out [][]byte
	if previous := st.OutputIndexes.lastBlock(); st.blockClosed(previous) {
		out = st.finalizeBlock(previous, "completed", nextSeq)
	}
	st.OutputIndexes.add(index)
	return out
}
...
func (st *claudeToResponsesState) finalizeBlock(idx int, status string, nextSeq func() int) [][]byte {
	...
	if block := st.TextBlocks[idx]; block != nil {
		...
		done, _ = sjson.SetBytes(done, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		partDone, _ = sjson.SetBytes(partDone, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		final, _ = sjson.SetBytes(final, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		if st.ToolIdentities[name].custom {
			...
			if value != "" {
				...
				delta, _ = sjson.SetBytes(delta, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		fcDone, _ = sjson.SetBytes(fcDone, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		itemDone, _ = sjson.SetBytes(itemDone, "output_index", st.OutputIndexes.ByBlock[idx])
		...
		if !block.Redacted {
			...
			textDone, _ = sjson.SetBytes(textDone, "output_index", st.OutputIndexes.ByBlock[idx])
