...
type claudeToResponsesState struct {
	...
	ResponseID    string
	CreatedAt     int64
	TextBlocks    map[int]*claudeResponsesTextBlock
	Completed     bool
	ItemDone      map[int]bool
...
func ConvertClaudeResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	if *param == nil {
		*param = &claudeToResponsesState{FuncArgsBuf: make(map[int]*strings.Builder), FuncNames: make(map[int]string), FuncCallIDs: make(map[int]string), FuncDone: make(map[int]bool)}
		...
		(*param).(*claudeToResponsesState).ItemDone = make(map[int]bool)
	}
	...
	var out [][]byte
	if st.Completed && ev != "message_start" {
		return out
	}
	idx := int(root.Get("index").Int())
	if claudeResponsesRequiresBlockIndex(root, len(st.FuncCallIDs) > 0, st.ReasoningBlocks[0] != nil) {
		var valid bool
		idx, valid = claudeResponsesBlockIndex(root.Get("index"))
		if !valid {
			return out
		}
	}
	...
	case "message_start":
		if msg := root.Get("message"); msg.Exists() {
			...
			st.Completed = false
			st.ItemDone = make(map[int]bool)
			st.OutputIndexes = claudeResponsesOutputIndexes{}
			st.StopReason = ""
	...
	case "content_block_start":
		...
		if typ == "text" {
			out = append(out, st.noteOutputIndex(idx, nextSeq)...)
			// open message item + content part
			block := &claudeResponsesTextBlock{ID: fmt.Sprintf("msg_%s_%d", st.ResponseID, idx)}
			st.TextBlocks[idx] = block
			item := []byte(`{"type":"response.output_item.added","sequence_number":0,"output_index":0,"item":{"id":"","type":"message","status":"in_progress","content":[],"role":"assistant"}}`)
			item, _ = sjson.SetBytes(item, "sequence_number", nextSeq())
			item, _ = sjson.SetBytes(item, "output_index", st.OutputIndexes.ByBlock[idx])
			...
			part, _ = sjson.SetBytes(part, "output_index", st.OutputIndexes.ByBlock[idx])
			part, _ = sjson.SetBytes(part, "item_id", block.ID)
			out = append(out, emitEvent("response.content_part.added", part))
		} else if typ == "tool_use" {
			if _, exists := st.FuncCallIDs[idx]; exists {
				break
			}
			...
			item := []byte(`{"type":"response.output_item.added","sequence_number":0,"output_index":0}`)
			item, _ = sjson.SetBytes(item, "sequence_number", nextSeq())
			item, _ = sjson.SetBytes(item, "output_index", st.OutputIndexes.ByBlock[idx])
			item, _ = sjson.SetRawBytes(item, "item", buildClaudeResponsesToolItem(st.ToolIdentities, name, callID, "", "in_progress"))
			out = append(out, emitEvent("response.output_item.added", item))
			...
			block := newClaudeResponsesReasoningBlock(fmt.Sprintf("rs_%s_%d", st.ResponseID, idx), cb)
			...
			item, _ = sjson.SetBytes(item, "output_index", st.OutputIndexes.ByBlock[idx])
			item, _ = sjson.SetBytes(item, "item.id", block.ID)
			item, _ = sjson.SetBytes(item, "item.encrypted_content", block.Signature.String())
			out = append(out, emitEvent("response.output_item.added", item))
			if block.Redacted {
				break
			}
			...
			if block.Text.Len() > 0 {
				out = append(out, claudeResponsesReasoningDelta(block, st.OutputIndexes.ByBlock[idx], block.Text.String(), nextSeq()))
			}
		}
	case "content_block_delta":
		...
		if dt == "text_delta" {
			...
			if t := d.Get("text"); t.Exists() {
				...
				msg, _ = sjson.SetBytes(msg, "output_index", st.OutputIndexes.ByBlock[idx])
			...
			if _, exists := st.FuncCallIDs[idx]; !exists || st.FuncDone[idx] {
				break
			}
			if pj := d.Get("partial_json"); pj.Exists() {
				...
				if st.ToolIdentities[st.FuncNames[idx]].custom {
					break
				}
			...
			if block := st.ReasoningBlocks[idx]; block != nil && !block.Done && !block.Redacted {
				if t := d.Get("thinking"); t.Type == gjson.String {
					block.Text.WriteString(t.String())
					out = append(out, claudeResponsesReasoningDelta(block, st.OutputIndexes.ByBlock[idx], t.String(), nextSeq()))
				}
			}
		} else if dt == "signature_delta" {
			if block := st.ReasoningBlocks[idx]; block != nil && !block.Done && !block.Redacted {
				if signature := d.Get("signature"); signature.Type == gjson.String {
	...
	case "content_block_stop":
		st.closeBlock(idx)
		if idx != st.OutputIndexes.lastBlock() {
			out = append(out, st.finalizeBlock(idx, "completed", nextSeq)...)
		}
	case "message_delta":
	...
	case "message_stop":
		...
		for _, index := range st.pendingBlockIndexes() {
			status := "completed"
			if index == st.OutputIndexes.lastBlock() {
				status = responseStatus
			}
			out = append(out, st.finalizeBlock(index, status, nextSeq)...)
		}
		st.Completed = true
		...
		for index := range st.FuncCallIDs {
			outputItems[index] = st.outputItem(index, "completed")
		}
		if last := st.OutputIndexes.lastBlock(); last >= 0 {
			outputItems[last] = st.outputItem(last, responseStatus)
		}
...
func (st *claudeToResponsesState) pendingBlockIndexes() []int {
	var indexes []int
	for _, index := range st.OutputIndexes.Blocks {
		if !st.ItemDone[index] {
...
func claudeResponsesReasoningItem(block *claudeResponsesTextBlock) []byte {
	...
	item, _ = sjson.SetBytes(item, "encrypted_content", block.Signature.String())
	if block.Redacted {
...
func claudeResponsesReasoningDelta(block *claudeResponsesTextBlock, index int, text string, sequence int) []byte {
	event := []byte(`{"type":"response.reasoning_summary_text.delta","sequence_number":0,"item_id":"","output_index":0,"summary_index":0,"delta":""}`)
	event, _ = sjson.SetBytes(event, "sequence_number", sequence)
	event, _ = sjson.SetBytes(event, "item_id", block.ID)
	event, _ = sjson.SetBytes(event, "output_index", index)
	event, _ = sjson.SetBytes(event, "delta", text)
	return emitEvent("response.reasoning_summary_text.delta", event)
}

func claudeResponsesOrderedOutput(items map[int][]byte, indexes []int) []byte {
	out := []byte{'['}
...
func ConvertClaudeResponseToOpenAIResponsesNonStream(_ context.Context, _ string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, _ *any) []byte {
	...
	for _, ch := range chunks {
		...
		ev := root.Get("type").String()
		if completed && ev != "message_start" {
			continue
		}
		idx := int(root.Get("index").Int())
		if claudeResponsesRequiresBlockIndex(root, len(toolCalls) > 0, reasoningBlocks[0] != nil) {
			var valid bool
			idx, valid = claudeResponsesBlockIndex(root.Get("index"))
			if !valid {
				continue
		...
		case "content_block_start":
			...
			case "text":
				...
				textBlocks[idx] = &claudeResponsesTextBlock{ID: fmt.Sprintf("msg_%s_%d", responseID, idx)}
			case "tool_use":
				callID := cb.Get("id").String()
				if toolCalls[idx] == nil && callID != "" {
					outputIndexes.add(idx)
					toolCalls[idx] = &toolState{id: callID, name: cb.Get("name").String()}
				}
			case "thinking", "redacted_thinking":
				outputIndexes.add(idx)
				reasoningBlocks[idx] = newClaudeResponsesReasoningBlock(fmt.Sprintf("rs_%s_%d", responseID, idx), cb)
			}
		...
		case "content_block_delta":
			...
			case "text_delta":
				...
				if block == nil {
					outputIndexes.add(idx)
					block = &claudeResponsesTextBlock{ID: fmt.Sprintf("msg_%s_%d", responseID, idx)}
					textBlocks[idx] = block
				}
				if t := d.Get("text"); t.Exists() {
					if !block.Done {
						block.Text.WriteString(t.String())
			...
			case "input_json_delta":
				if pj := d.Get("partial_json"); pj.Exists() {
					if call := toolCalls[idx]; call != nil && !call.done {
						call.args.WriteString(pj.String())
					}
				}
			case "thinking_delta":
				if block := reasoningBlocks[idx]; block != nil && !block.Done && !block.Redacted {
					if t := d.Get("thinking"); t.Type == gjson.String {
						block.Text.WriteString(t.String())
					}
				}
	...
	lastIndex := outputIndexes.lastBlock()
