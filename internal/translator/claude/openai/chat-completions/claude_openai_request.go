...
// as unsigned thinking history for configured compatibility endpoints.
		...
		messages.ForEach(func(_, message gjson.Result) bool {
			...
			case "user", "assistant":
				...
				if preserveThinking && role == "assistant" {
					if text := message.Get("reasoning_content"); text.Type == gjson.String && strings.TrimSpace(text.String()) != "" {
						part := []byte(`{"type":"thinking","thinking":"","signature":""}`)
						part, _ = sjson.SetBytes(part, "thinking", text.String())
			...
			case "tool":
				...
				if toolCallID != "" {
					if emittedToolResults[toolCallID] {
						return true
					}
					emittedToolResults[toolCallID] = true
					targetMessage = lastToolMessage[toolCallID]
				}
				toolContentResult := targetMessage.Get("content")
				...
				if cache := targetMessage.Get("cache_control"); cache.IsObject() {
					msg, _ = sjson.SetRawBytes(msg, "content.0.cache_control", []byte(cache.Raw))
				}
				out, _ = sjson.SetRawBytes(out, "messages.-1", msg)
				messageIndex++
			}
