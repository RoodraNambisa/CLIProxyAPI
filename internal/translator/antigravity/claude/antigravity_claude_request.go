...
func ConvertClaudeRequestToAntigravity(modelName string, inputRawJSON []byte, _ bool) []byte {
	...
	var toolsJSON []byte
	toolDeclCount := 0
	removedTypedToolNames := make(map[string]struct{})
	...
	if toolsResult.IsArray() {
		...
		for i := 0; i < len(toolsResults); i++ {
			...
			if isClaudeTypedWebSearchToolType(toolResult.Get("type").String()) {
				removedTypedToolNames["web_search"] = struct{}{}
				if name := util.SanitizeFunctionName(toolResult.Get("name").String()); name != "" {
					removedTypedToolNames[name] = struct{}{}
				}
				continue
			}
	...
	if toolChoiceResult.Exists() && toolDeclCount > 0 {
		toolChoiceType := ""
		toolChoiceName := ""
		...
		sanitizedToolChoiceName := util.SanitizeFunctionName(toolChoiceName)
		_, selectedRemovedTool := removedTypedToolNames[sanitizedToolChoiceName]
		if toolChoiceType == "tool" && selectedRemovedTool {
		...
		case "tool":
			out, _ = sjson.SetBytes(out, "request.toolConfig.functionCallingConfig.mode", "ANY")
			if sanitizedToolChoiceName != "" {
				out, _ = sjson.SetBytes(out, "request.toolConfig.functionCallingConfig.allowedFunctionNames", []string{sanitizedToolChoiceName})
			}
		}
	}

	// Map the Anthropic thinking amount without inferring summary visibility.
