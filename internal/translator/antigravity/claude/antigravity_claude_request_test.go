...
func TestConvertClaudeRequestToAntigravity_ConvertsMessageSystemRoleToUserContent(t *testing.T) {
	...
	if systemContent := gjson.GetBytes(output, `request.contents.#(role=="system")`); systemContent.Exists() {
		t.Fatalf("system role remained in contents: %s", systemContent.Raw)
	}
	contents := gjson.GetBytes(output, "request.contents").Array()
	if len(contents) != 3 {
		t.Fatalf("contents = %d, want 3: %s", len(contents), output)
	}
	for i := range contents {
		if got := contents[i].Get("role").String(); got != "user" {
			t.Fatalf("contents[%d].role = %q", i, got)
		}
	}
	if got := contents[1].Get("parts.0.text").String(); got != "<system-reminder>\nString mid-conversation rule\n</system-reminder>" {
		t.Fatalf("string system reminder = %q", got)
	}
	if got := contents[2].Get("parts.0.text").String(); got != "<system-reminder>\nArray mid-conversation rule\n</system-reminder>" {
		t.Fatalf("array system reminder = %q", got)
	}
	if got := gjson.GetBytes(output, "request.systemInstruction.parts.0.text").String(); got != "Top-level rules" {
		t.Fatalf("top-level system = %q", got)
	}
}
...
func TestConvertClaudeRequestToAntigravity_ThinkingConfig(t *testing.T) {
	...
	if thinkingConfig.Exists() {
		...
		if thinkingConfig.Get("includeThoughts").Exists() {
			t.Error("thinking amount must not imply summary visibility")
		}
	} else {
