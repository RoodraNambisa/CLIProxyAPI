...
func TestConvertGeminiRequestToAntigravity_ClaudeSignatureSanitizerPreservesLargeIntegers(t *testing.T) {
	inputJSON := []byte(`{
		"model": "claude-opus-4-6-thinking",
		"contents": [
			{
	...
	output := ConvertGeminiRequestToAntigravity("claude-opus-4-6-thinking", inputJSON, false)
	if got := gjson.GetBytes(output, "request.contents.0.parts.0.functionCall.args.account_id").Raw; got != "9007199254740993" {
		t.Fatalf("account_id = %s, want 9007199254740993. Output: %s", got, output)
	}
}
