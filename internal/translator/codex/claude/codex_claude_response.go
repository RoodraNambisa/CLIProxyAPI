...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	if typeStr == "response.created" {
		...
		// Wait for output_item.done to obtain the final encrypted_content.
...
func ConvertCodexResponseToClaudeNonStream(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, _ *any) []byte {
	...
	out, _ = sjson.SetBytes(out, "stop_reason", mapCodexStopReasonToClaude(responseData, hasToolCall))
...
func stopCodexTextBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	...
	output := appendCodexFunctionCallStop(nil, params.BlockIndex)
...
func appendCodexThinkingDelta(output []byte, params *ConvertCodexResponseToClaudeParams, text string) []byte {
	if text == "" {
		return output
	}
