...
func appendCodexWebSearchServerToolUse(output []byte, params *ConvertCodexResponseToClaudeParams, root, item gjson.Result) []byte {
	...
	_, alreadyStarted := params.WebSearchToolUseIDs[toolUseID]
	if alreadyStarted && query == "" {
		return output
	}
...
func appendCodexWebSearchToolResult(output []byte, params *ConvertCodexResponseToClaudeParams, root, item gjson.Result) []byte {
	...
	content := codexWebSearchResultContent(root, item)
	if codexWebSearchQuery(root, item) == "" && len(content) == 0 && !item.Get("action").Exists() {
		return output
	}
...
func codexWebSearchToolUseID(params *ConvertCodexResponseToClaudeParams, root, item gjson.Result) string {
	...
	if value := strings.TrimSpace(item.Get("item_id").String()); value != "" {
		return value
	}
	if value := strings.TrimSpace(root.Get("item_id").String()); value != "" {
		return value
	}
