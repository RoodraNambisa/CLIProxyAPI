...
func TestRefreshTokensUsesIndependentTimeout(t *testing.T) {
	...
	remaining := requestDeadline.Sub(startedAt)
	if remaining < codexRefreshTimeout-time.Second || remaining > codexRefreshTimeout+time.Second {
		t.Fatalf("refresh deadline remaining = %s, want approximately %s", remaining, codexRefreshTimeout)
	}
}
