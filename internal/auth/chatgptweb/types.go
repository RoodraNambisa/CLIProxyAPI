...
type Credential struct {
	...
	AdvancedAccountSecurity  *AdvancedAccountSecurityCredential `json:"advanced_account_security,omitempty"`
...
func DecodeCredential(data []byte) (*Credential, error) {
	...
	} else if credential.CredentialSchemaVersion == CredentialSchemaVersionAdvancedAccountSecurity {
		if err := validateCredentialAdvancedAccountSecurityJSON(data); err != nil {
			return nil, err
		}
	}
	...
	if credential.CredentialSchemaVersion == CredentialSchemaVersionAdvancedAccountSecurity {
		loginMethod = LoginMethodAdvancedSecurityPasskey
	}
...
func (credential *Credential) ApplyToMetadata(metadata map[string]any) {
	...
	if credential.AdvancedAccountSecurity == nil {
		delete(metadata, "advanced_account_security")
	} else {
		metadata["advanced_account_security"] = CloneAdvancedAccountSecurityCredential(credential.AdvancedAccountSecurity)
	}
	loginMethod, errLoginMethod := NormalizeLoginMethod(credential.LoginMethod)
	if errLoginMethod != nil {
...
type LoginInput struct {
	...
	PersistAdvancedAccountSecurity func(context.Context, AdvancedAccountSecurityCredential) (AdvancedAccountSecurityCredential, error)
