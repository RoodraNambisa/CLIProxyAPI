...
func (e *XAIExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		clearXAIReasoningReplayOnInvalidSignature(ctx, prepared.replayScope, httpResp.StatusCode, data)
		return resp, xaiStatusErrFromResponse(httpResp.StatusCode, data, httpResp.Header, time.Now())
	}
...
func (e *XAIExecutor) executeCompactRequest(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*xaiPreparedRequest, []byte, http.Header, error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), data))
		err = xaiStatusErrFromResponse(httpResp.StatusCode, data, httpResp.Header, time.Now())
		return nil, nil, nil, err
	}
...
func (e *XAIExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if httpResp.StatusCode < 200 || httpResp.StatusCode >= 300 {
		...
		helps.LogWithRequestID(ctx).Debugf("request error, error status: %d, error message: %s", httpResp.StatusCode, helps.SummarizeErrorBody(httpResp.Header.Get("Content-Type"), data))
		clearXAIReasoningReplayOnInvalidSignature(ctx, prepared.replayScope, httpResp.StatusCode, data)
		return nil, xaiStatusErrFromResponse(httpResp.StatusCode, data, httpResp.Header, time.Now())
	}
...
func (e *XAIExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	svc := xaiauth.NewXAIAuthWithProxyURL(e.cfg, auth.EffectiveProxyURL())
...
func (e *XAIExecutor) prepareResponsesRequestTo(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, stream bool, to sdktranslator.Format) (*xaiPreparedRequest, error) {
	...
	body, replayScope, err = applyXAIReasoningReplayCacheRequired(ctx, replayNamespace, from, req, opts, body)
	if err != nil {
		return nil, err
	}
...
func xaiResolveComposerSessionID(ctx context.Context, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, baseModel string) (string, error) {
	...
	if sessionID := xaiClaudeCodeSessionID(req.Payload, opts.Headers); sessionID != "" {
		return helps.CachedSessionID("xai-composer:" + baseModel + ":" + sessionID), nil
	}
	return uuid.NewString(), nil
}

func xaiClaudeCodeSessionID(payload []byte, headers http.Header) string {
	if headers != nil {
...
func normalizeXAIToolChoiceForTools(body []byte) []byte {
	...
	if hasTools {
		...
		if !choice.Exists() || choice.Type != gjson.JSON || !choice.IsObject() {
			return body
		}
		...
		for _, tool := range tools.Array() {
			...
			if choiceType == xaiCustomToolType && toolType == xaiFunctionToolType && choiceName != "" && choiceName == toolName {
				body, _ = sjson.SetBytes(body, "tool_choice.type", xaiFunctionToolType)
				return body
			}
...
func xaiStatusErrFromResponse(code int, body []byte, headers http.Header, now time.Time) statusErr {
	err := xaiStatusErr(code, body)
	if err.retryAfter != nil || code != http.StatusTooManyRequests || headers == nil {
		return err
	}
	err.retryAfter = parseXAIRetryAfterHeader(headers.Get("Retry-After"), now)
	return err
}

func parseXAIRetryAfterHeader(value string, now time.Time) *time.Duration {
...
func xaiFreeUsageExhausted(body []byte) bool {
	...
	return strings.Contains(codeStr, "free-usage-exhausted") ||
		strings.Contains(msg, "free-usage-exhausted") ||
		strings.Contains(msg, "included free usage")
}

func xaiTerminalStreamError(eventData []byte) (statusErr, bool) {
	err, ok := codexTerminalStreamError(eventData)
	if !ok {
		return statusErr{}, false
	}
	if xaiFreeUsageExhausted(eventData) {
		err.code = http.StatusTooManyRequests
		d := xaiFreeUsageExhaustedCooldown
		err.retryAfter = &d
	} else if err.code == http.StatusTooManyRequests {
		xaiErr := xaiStatusErr(err.code, eventData)
		err.retryAfter = xaiErr.retryAfter
	}
	return err, true
}
