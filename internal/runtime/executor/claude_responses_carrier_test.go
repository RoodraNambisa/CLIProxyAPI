...
func TestClaudeResponsesIncompleteReachesClient(t *testing.T) {
...
`)
				...
				var result gjson.Result
				if stream {
					response, err := manager.ExecuteStream(t.Context(), []string{"claude"}, req, opts)
					if err != nil {
						t.Fatal(err)
					}
					...
					response, err := manager.Execute(t.Context(), []string{"claude"}, req, opts)
					if err != nil {
						t.Fatal(err)
					}
