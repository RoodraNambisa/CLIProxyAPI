...
func (*ClaudeExecutor) DeferAuthRequestCommitUntilUpstream() bool { return true }
...
func (e *ClaudeExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	originalTranslated, err := helps.TranslateRequestWithCodexMultiAgentV2(ctx, opts.Headers, e.cfg, from, to, baseModel, originalPayload, stream, helps.APIKeyModelIsCompat(req))
	if err != nil {
		return resp, err
	}
	body, err := helps.TranslateRequestWithCodexMultiAgentV2(ctx, opts.Headers, e.cfg, from, to, baseModel, req.Payload, stream, helps.APIKeyModelIsCompat(req))
	if err != nil {
		return resp, err
	}
...
func (e *ClaudeExecutor) Refresh(ctx context.Context, auth *cliproxyauth.Auth) (*cliproxyauth.Auth, error) {
	...
	svc := claudeauth.NewClaudeAuthWithProxyURL(e.cfg, auth.EffectiveProxyURL())
...
func applyCloaking(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth, payload []byte, model string, apiKey string) ([]byte, error) {
	...
	shouldCloak := helps.ShouldCloak(cloakMode, clientUserAgent)
	discardsSystem := shouldCloak && strictMode && !strings.HasPrefix(model, "claude-3-5-haiku") &&
		!strings.HasPrefix(gjson.GetBytes(payload, "system.0.text").String(), "x-anthropic-billing-header:")
	if !discardsSystem {
		if errSystem := helps.ValidateClaudeSystemInputs(ctx, payload); errSystem != nil {
