...
func antigravityReasoningReplayResolveContentIndex(payload []byte, cached int) (int, bool) {
	contents := gjson.GetBytes(payload, "request.contents")
	if !contents.IsArray() {
		return 0, false
	}
	arr := contents.Array()
	if cached >= 0 && cached < len(arr) && strings.EqualFold(strings.TrimSpace(arr[cached].Get("role").String()), "model") {
		return cached, true
	}
...
func insertAntigravityModelFunctionCallBeforeContent(payload []byte, beforeIndex int, name, callID, thoughtSig string, args gjson.Result) ([]byte, bool) {
	...
	if callID != "" {
		fc, _ = sjson.SetBytes(fc, "id", callID)
	}
	if args.Exists() {
		if args.Type == gjson.String {
			fc, _ = sjson.SetBytes(fc, "args", args.String())
		} else if json.Valid([]byte(args.Raw)) {
			fc, _ = sjson.SetRawBytes(fc, "args", []byte(args.Raw))
		}
	}
	part := []byte(`{"functionCall":{}}`)
	part, _ = sjson.SetRawBytes(part, "functionCall", fc)
	if thoughtSig != "" {
		part, _ = sjson.SetBytes(part, "thoughtSignature", thoughtSig)
	}
	newContent := []byte(`{"role":"model","parts":[]}`)
	newContent, _ = sjson.SetRawBytes(newContent, "parts.-1", part)
	newArr := make([]json.RawMessage, 0, len(arr)+1)
	for i := 0; i < beforeIndex; i++ {
		newArr = append(newArr, json.RawMessage(arr[i].Raw))
	}
	newArr = append(newArr, json.RawMessage(newContent))
	for i := beforeIndex; i < len(arr); i++ {
		newArr = append(newArr, json.RawMessage(arr[i].Raw))
	}
	newArrJSON, err := json.Marshal(newArr)
	if err != nil {
		return payload, false
	}
	updated, err := sjson.SetRawBytes(payload, "request.contents", newArrJSON)
	if err != nil {
		return payload, false
	}
	return updated, true
...
func insertAntigravityReasoningReplayItems(payload []byte, items [][]byte) ([]byte, bool) {
	...
	for _, item := range items {
		...
		switch strings.TrimSpace(itemResult.Get("type").String()) {
		case "thought_signature":
			ci, okResolve := antigravityReasoningReplayResolveContentIndex(out, int(itemResult.Get("contentIndex").Int()))
			if !okResolve {
				continue
			}
			pi := int(itemResult.Get("partIndex").Int())
...
func mergeAntigravityFunctionCallPartReplay(payload []byte, itemResult gjson.Result) ([]byte, bool) {
	...
	ci, okResolve := antigravityReasoningReplayResolveContentIndex(payload, int(itemResult.Get("contentIndex").Int()))
	if !okResolve {
		return payload, false
	}
	pi := int(itemResult.Get("partIndex").Int())
	...
	if !exists {
		...
		if args.Type == gjson.String {
			...
			fc["args"] = json.RawMessage(args.Raw)
		}
