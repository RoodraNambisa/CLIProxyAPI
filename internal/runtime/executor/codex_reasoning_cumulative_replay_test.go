...
func TestCodexHTTPRestoresCumulativeReasoningOnlyFromCompletedTurns(t *testing.T) {
		...
		for _, release := range []bool{false, true} {
			t.Run(fmt.Sprintf("stream=%t/release=%t", stream, release), func(t *testing.T) {
				...
				upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
					...
					_, _ = fmt.Fprintf(w, "data: {\"type\":\"response.output_item.done\",\"output_index\":0,\"item\":{\"type\":\"reasoning\",\"encrypted_content\":%q}}\n\n", signature(byte(turn)))
					_, _ = fmt.Fprintf(w, "data: {\"type\":\"response.output_item.done\",\"output_index\":1,\"item\":{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"answer%d\"}]}}\n\n", turn)
					errorJSON := "null"
					if call == 4 {
						errorJSON = `{"code":"misalignment_policy_violation","message":"fixture rejection"}`
					}
					_, _ = fmt.Fprintf(w, "data: {\"type\":\"response.%s\",\"response\":{\"status\":%q,\"error\":%s,\"incomplete_details\":{\"reason\":\"max_tokens\"},\"output\":[]}}\n\n", status, status, errorJSON)
				}))
