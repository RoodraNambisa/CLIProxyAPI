	...
	for _, transport := range []string{"http", "sse", "incomplete", "websocket"} {
		for _, field := range []string{"code", "type"} {
			for _, enabled := range []bool{false, true} {
				t.Run(fmt.Sprintf("%s/%s/buffering=%t", transport, field, enabled), func(t *testing.T) {
					...
					defer server.Close()
					cfg := &config.Config{SDKConfig: sdkconfig.SDKConfig{ProxyURL: "direct"}, Codex: config.CodexConfig{StreamBootstrapBuffering: enabled}}
