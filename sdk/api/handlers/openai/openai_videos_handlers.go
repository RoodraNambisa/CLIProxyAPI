...
func isXAIVideosModel(model string) bool {
	...
	return baseModel == defaultXAIVideosModel || baseModel == xaiVideos15PreviewModel
}
...
func xaiVideosRouteModel(model string) string {
	prefix, _ := imagesModelParts(model)
	canonical := canonicalXAIVideosModel(model)
	if prefix == "" {
		return canonical
	}
...
func (h *OpenAIAPIHandler) bindVideoAuthIDAndModelFromPayload(payload []byte, authID string, model string) {
	...
	videoAuthBindings.setWithModel(videoID, authID, xaiVideosRouteModel(model), h.videoAuthBindingTTL())
}
...
func (h *OpenAIAPIHandler) handleXAIVideosNativePost(c *gin.Context) {
	...
	upstreamPayload, _ := sjson.SetBytes(rawJSON, "model", canonicalXAIVideosModel(videoModel))
	h.collectXAIVideosNative(c, upstreamPayload, videoModel, true)
}
...
func (h *OpenAIAPIHandler) VideosRetrieve(c *gin.Context) {
	...
	resp, upstreamHeaders, errMsg := h.ExecuteWithProviders(cliCtx, []string{xaiProvider}, xaiVideosHandlerType, executionModel, payload, "")
	stopKeepAlive()
	if errMsg != nil {
...
func (h *OpenAIAPIHandler) VideosContent(c *gin.Context) {
	...
	stopKeepAlive := h.StartNonStreamingKeepAlive(c, cliCtx)
	resp, _, errMsg := h.ExecuteWithProviders(cliCtx, []string{xaiProvider}, xaiVideosHandlerType, executionModel, payload, "")
	stopKeepAlive()
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
		if errMsg.Error != nil {
...
func (h *OpenAIAPIHandler) collectXAIVideosNative(c *gin.Context, rawJSON []byte, model string, bindCreatedVideoAuth bool) {
	...
	stopKeepAlive := h.StartNonStreamingKeepAlive(c, cliCtx)
	resp, upstreamHeaders, errMsg := h.ExecuteWithProviders(cliCtx, []string{xaiProvider}, xaiVideosHandlerType, executionModel, rawJSON, "")
	stopKeepAlive()
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
		if errMsg.Error != nil {
...
func (h *OpenAIAPIHandler) collectXAIVideosCreate(c *gin.Context, xaiReq []byte, meta xaiVideoCreateMetadata) {
	...
	stopKeepAlive := h.StartNonStreamingKeepAlive(c, cliCtx)
	resp, upstreamHeaders, errMsg := h.ExecuteWithProviders(cliCtx, []string{xaiProvider}, xaiVideosHandlerType, upstreamModel, xaiReq, "")
	stopKeepAlive()
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
		if errMsg.Error != nil {
	...
	h.bindVideoAuthIDAndModelFromPayload(out, selectedAuthID, upstreamModel)
	handlers.WriteUpstreamHeaders(c.Writer.Header(), upstreamHeaders)
	_, _ = c.Writer.Write(out)
	cliCancel(nil)
}
