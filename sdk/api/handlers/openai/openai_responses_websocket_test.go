...
func TestForwardResponsesWebsocketTreatsFailedEventAsTerminal(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		data := make(chan []byte, 1)
		errCh := make(chan *interfaces.ErrorMessage)
		data <- []byte("data: {\"type\":\"response.failed\",\"response\":{\"id\":\"resp-1\",\"status\":\"failed\",\"error\":{\"code\":\"server_error\",\"message\":\"failed\"},\"output\":[]}}\n\n")
		close(data)
		close(errCh)
		...
		if strings.Contains(timelineLog.String(), "stream closed before response.completed") {
			serverErrCh <- errors.New("terminal failed event was followed by stream closed error")
			return
		}
		serverErrCh <- nil
	}))
	...
	if gjson.GetBytes(payload, "type").String() != wsEventTypeFailed {
		t.Fatalf("payload type = %s, want %s", gjson.GetBytes(payload, "type").String(), wsEventTypeFailed)
	}
...
func TestResponsesWebsocketPrewarmEnforcesProviderRestriction(t *testing.T) {
	gin.SetMode(gin.TestMode)

	executor := &websocketCaptureExecutor{}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)
	auth := &coreauth.Auth{ID: "auth-restricted-prewarm", Provider: executor.Identifier(), Status: coreauth.StatusActive}
	if _, err := manager.Register(context.Background(), auth); err != nil {
		t.Fatalf("Register auth: %v", err)
	}
	modelID := "restricted-prewarm-model"
	...
	request := fmt.Sprintf(`{"type":"response.create","model":%q,"generate":false}`, modelID)
	if errWrite := conn.WriteMessage(websocket.TextMessage, []byte(request)); errWrite != nil {
		t.Fatalf("write prewarm request: %v", errWrite)
	}
	_, payload, errRead := conn.ReadMessage()
	if errRead != nil {
		t.Fatalf("read prewarm error: %v", errRead)
	}
	if eventType := gjson.GetBytes(payload, "type").String(); eventType != wsEventTypeError {
		t.Fatalf("event type = %q, want error; payload=%s", eventType, payload)
	}
	if status := gjson.GetBytes(payload, "status").Int(); status != http.StatusForbidden {
		t.Fatalf("status = %d, want 403; payload=%s", status, payload)
	}
	if code := gjson.GetBytes(payload, "error.code").String(); code != "provider_not_allowed" {
		t.Fatalf("error code = %q, want provider_not_allowed; payload=%s", code, payload)
	}
	if executor.streamCalls != 0 {
...
func TestResponsesWebsocketDownstreamSessionKeyAcceptsHyphenCodexSessionID(t *testing.T) {
	req := httptest.NewRequest(http.MethodGet, "/v1/responses/ws", nil)
	req.Header.Set("Session-Id", "codex-session-hyphen")

	if got := websocketDownstreamSessionKey(req); got != "codex-session-hyphen" {
		t.Fatalf("websocketDownstreamSessionKey() = %q, want codex-session-hyphen", got)
	}
}
...
func TestNormalizeResponsesWebsocketRequestDropsDuplicateInputItemsByID(t *testing.T) {
	lastRequest := []byte(`{"model":"test-model","stream":true,"input":[{"type":"message","id":"msg-1"},{"type":"message","id":"msg-2"}]}`)
	lastResponseOutput := []byte(`[
		{"type":"message","id":"msg-2"}
	]`)
	raw := []byte(`{"type":"response.create","input":[{"type":"message","id":"msg-2"},{"type":"message","id":"msg-3"}]}`)
...
func TestDedupeResponsesWebsocketInputItemsByIDAfterRepair(t *testing.T) {
	payload := []byte(`{"type":"response.create","input":[{"type":"message","id":"msg-1"},{"type":"message","id":"msg-1"},{"type":"function_call","id":"fc-1","call_id":"call-1"},{"type":"function_call","id":"fc-1","call_id":"call-1"}]}`)
	...
	if items[0].Get("id").String() != "msg-1" || items[1].Get("id").String() != "fc-1" {
		t.Fatalf("unexpected deduped input order: %s", deduped)
	}
}
