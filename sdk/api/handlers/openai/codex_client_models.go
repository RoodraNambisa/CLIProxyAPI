...
func applyCodexClientMaxTokens(entry map[string]any, model map[string]any) {
	if limit := intModelValue(model, "max_completion_tokens"); limit > 0 {
		entry["max_tokens"] = limit
	}
}
...
func applyCodexClientVisibilityOverride(entry map[string]any, id string) {
	...
	case "grok-imagine-image-quality", "gpt-image-1.5", "gpt-image-2", "gpt-image-2.5", "gpt-image-2.5-flare", "gpt-image-2.5-sunburst", "grok-imagine-image", "grok-imagine-video", "grok-imagine-video-1.5-preview":
		entry["visibility"] = "hide"
	}
}
