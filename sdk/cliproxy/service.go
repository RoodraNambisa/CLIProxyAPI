	...
	if s.coreManager != nil && previousCfg != nil && routingSelectorSettingsForConfig(previousCfg) == routingSelectorSettingsForConfig(nextCfg) {
		...
		if nextCfg.Routing.ClaudeCodeSessionAffinity || nextCfg.Routing.SessionAffinity {
			...
			selector = coreauth.NewSessionAffinitySelectorWithConfig(coreauth.SessionAffinityConfig{
				...
				UseHistory:       nextCfg.Routing.SessionAffinityUseHistory,
				LCP:              nextCfg.Routing.SessionAffinity && nextCfg.Routing.SessionAffinityLCP,
			})
		}
...
func (s *Service) registerModelsForAuthWithState(a *coreauth.Auth, preserveTransientState bool) {
	...
	case "xai":
		...
		if entry := s.resolveConfigXAIKey(a); entry != nil {
			if len(entry.Models) > 0 {
				models = buildConfigModels(entry.Models, "xai", "xai")
			}
			if authKind == "apikey" {
				excluded = entry.ExcludedModels
			}
