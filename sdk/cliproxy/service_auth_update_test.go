...
func TestApplyCoreAuthAddOrUpdateSkipsInstalledChatGPTWebFileGeneration(t *testing.T) {
	authDir := t.TempDir()
	path := filepath.Join(authDir, "chatgpt-web.json")
	data := []byte(`{"type":"chatgpt-web","email":"person@example.com","access_token":"access","refresh_strategy":"token_only","lifecycle_state":"active"}`)
	...
	if errSync := coreauth.SyncPersistedMetadataAndSourceHash(auth, data); errSync != nil {
		t.Fatalf("SyncPersistedMetadataAndSourceHash() error = %v", errSync)
	}
	manager := coreauth.NewManager(nil, nil, nil)
	...
	service.applyCoreAuthAddOrUpdate(t.Context(), auth.CloneWithoutRuntimeInstance())

	current, ok := manager.GetByID(auth.ID)
	if !ok || current == nil {
		t.Fatal("installed auth disappeared")
	}
	if current.RuntimeInstallationID() != registered.RuntimeInstallationID() ||
		current.RuntimeInstanceID() != registered.RuntimeInstanceID() {
		t.Fatalf(
			"watcher echo replaced runtime generation: installation %q -> %q, instance %q -> %q",
			registered.RuntimeInstallationID(),
...
func TestChatGPTWebAuthFileGenerationAlreadyInstalledRequiresSameFileAndHash(t *testing.T) {
	authDir := t.TempDir()
	path := filepath.Join(authDir, "chatgpt-web.json")
	current := &coreauth.Auth{
		...
		Provider:   "chatgpt-web",
		FileName:   path,
		Attributes: map[string]string{"path": path, coreauth.SourceHashAttributeKey: "source-a"},
	}
	incoming := current.CloneWithoutRuntimeInstance()
	if !chatGPTWebAuthFileGenerationAlreadyInstalled(current, incoming, authDir) {
		t.Fatal("same ChatGPT Web file generation was not recognized")
	}

	differentHash := incoming.CloneWithoutRuntimeInstance()
	differentHash.Attributes[coreauth.SourceHashAttributeKey] = "source-b"
	if chatGPTWebAuthFileGenerationAlreadyInstalled(current, differentHash, authDir) {
		t.Fatal("different source hash was treated as already installed")
	}

	differentPath := incoming.CloneWithoutRuntimeInstance()
	differentPath.FileName = filepath.Join(authDir, "other.json")
	differentPath.Attributes["path"] = differentPath.FileName
	if chatGPTWebAuthFileGenerationAlreadyInstalled(current, differentPath, authDir) {
		t.Fatal("different backing file was treated as already installed")
	}

	differentDerivedConfig := incoming.CloneWithoutRuntimeInstance()
	differentDerivedConfig.Attributes["excluded_models"] = "gpt-image-1"
	if chatGPTWebAuthFileGenerationAlreadyInstalled(current, differentDerivedConfig, authDir) {
		t.Fatal("different config-derived attributes were treated as already installed")
	}

	nonChatGPTWeb := incoming.CloneWithoutRuntimeInstance()
	nonChatGPTWeb.Provider = "codex"
	if chatGPTWebAuthFileGenerationAlreadyInstalled(current, nonChatGPTWeb, authDir) {
		t.Fatal("non-ChatGPT Web auth was treated as already installed")
	}
}
