...
func TestConfiguredModelCapabilityRoutesAreExactAndPrivate(t *testing.T) {
	models := []config.CodexModel{
		{Name: "upstream", Alias: "shared", Thinking: &registry.ThinkingSupport{Levels: []string{" LOW ", "none", "none"}}},
	...
	for _, tc := range []struct{ route, upstream, level string }{
		...
		if !ok || info.UserDefined || info.Thinking.Levels[0] != tc.level {
	...
	if !ok || !first.Thinking.ZeroAllowed || !reflect.DeepEqual(first.Thinking.Levels, []string{"low", "none"}) {
	...
	if second.ID != "upstream" || second.Thinking.Levels[0] != "low" {
...
func TestConfiguredModelCapabilityCompilerSelectsEachCredentialFamily(t *testing.T) {
	...
	cfg := &config.Config{
		GeminiKey:           []config.GeminiKey{{APIKey: "fixture", Models: []config.GeminiModel{{Name: "native", Alias: "local", Thinking: support}}}},
		InteractionsKey:     []config.GeminiKey{{APIKey: "fixture", BaseURL: "https://case.invalid", Models: []config.GeminiModel{{Name: "native", Alias: "local", Thinking: support}}}},
		ClaudeKey:           []config.ClaudeKey{{APIKey: "fixture", Models: []config.ClaudeModel{{Name: "native", Alias: "local", Thinking: support}}}},
		CodexKey:            []config.CodexKey{{APIKey: "fixture", Models: []config.CodexModel{{Name: "native", Alias: "local", Thinking: support}}}},
		VertexCompatAPIKey:  []config.VertexCompatKey{{APIKey: "fixture", Models: []config.VertexCompatModel{{Name: "native", Alias: "local", Thinking: support}}}},
	...
	for _, provider := range []string{"gemini", "gemini-interactions", "claude", "codex", "vertex", "compat"} {
		t.Run(provider, func(t *testing.T) {
			...
			if len(compiled["local"]) != 1 || !reflect.DeepEqual(compiled["local"][0].modelInfo.Thinking.Levels, []string{"high"}) {
				t.Fatal("selected credential definition not compiled")
			}
			if provider == "compat" {
				info := compiled["default"][0].modelInfo
				if info.UserDefined || !reflect.DeepEqual(info.Thinking.Levels, []string{"low", "medium", "high"}) {
